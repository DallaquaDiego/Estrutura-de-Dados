...
int imprime_folhas_decrescente ( ArvBin *raiz ) {
    ...
    // Verifica se a árvore ou o nó atual é nulo
    ...
    // Chama recursivamente a função para a subárvore direita (ordem decrescente)
    imprime_folhas_decrescente( ( *raiz ) -> dir );

    // Chama recursivamente a função para a subárvore esquerda (ordem decrescente)
    imprime_folhas_decrescente( ( *raiz ) -> esq );
