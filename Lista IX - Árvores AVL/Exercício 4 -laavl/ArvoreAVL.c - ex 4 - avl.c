/*4) Construa a função de rotação simples a esquerda void
...
void RotacaoRR(ArvAVL *raiz){
    printf("RotaçãoRR\n");
    struct NO *B;
    B = (*raiz)->dir; //B passa a apontar para o nó a direita da raiz
    (*raiz)->dir = B->esq; //ponteiro já passa a apontar para a esquerda (mesmo que ainda não tenha feito a troca de fato)
    B->esq = *raiz; //agora é feita a correção, onde a esqueda do B fica de fato a raiz 
    (*raiz)->altura = maior(altura_NO((*raiz)->dir), altura_NO((*raiz)->esq))+1; 
    B->altura = maior (altura_NO(B->dir), (*raiz)->altura) +1; //calcula a diferença e atualiza ela 
    *raiz=B; //atualiza qual é agora a raiz
}