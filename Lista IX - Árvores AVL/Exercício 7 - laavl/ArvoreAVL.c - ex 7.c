...
int verificaAVL(ArvAVL *raiz) {
    ...
    if (*raiz == NULL) {//se a árvore tiver vazia, é AVL por definição
        return 1;
    }

    int balanceamento = fatorBalanceamento_NO(*raiz); //calcula o fator de balanceamento para o nó atual

    if (balanceamento > 1 || balanceamento < -1) {// verifica se o balanceamento tá dentro dos limites permitidos
        return 0;  //não é uma AVL
    }

    if (!verificaAVL(&((*raiz)->esq)) || !verificaAVL(&((*raiz)->dir))) { //verifica recursivamente pros nós da subárvore esquerda e da direita
        return 0;  //não é uma AVL
    }
    
    return 1; //se deu tudo certo nesses passos, então é AVL
}
